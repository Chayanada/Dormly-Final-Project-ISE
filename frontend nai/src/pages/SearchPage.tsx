...
//   1: 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1470&q=80',
//   2: 'https://images.pexels.com/photos/271639/pexels-photo-271639.jpeg',
//   3: 'https://images.pexels.com/photos/323780/pexels-photo-323780.jpeg',
//   // ...เพิ่มตามต้องการ
// };

// // รูป default ถ้าหาไม่เจอใน map
...
const DORM_IMAGE_MAP: Record<number, string> = {
  ...
  10:'https://images.unsplash.com/photo-1598928506311-c55ded91a20c?auto=format&fit=crop&w=1470&q=80',
};
...
function DormCard({ dorm, showDistance }: DormCardProps) {
  ...
  // ใช้เฉพาะ map + default ไม่อิง backend cover_image แล้ว
  const imageSrc = DORM_IMAGE_MAP[dorm.dorm_id] || DEFAULT_DORM_IMAGE;

  // ลอง console ดู dorm_id ว่าตรงกับ map ไหม
  console.log('Dorm:', dorm.dorm_id, dorm.dorm_name, '->', imageSrc);
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-2">
          <p
            className="text-gray-600 text-sm truncate mr-2"
            title={dorm.address}
          >
            {dorm.address}
          </p>
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <span
              className={`w-2 h-2 rounded-full ${
                dorm.available_rooms > 0 ? 'bg-green-500' : 'bg-red-500'
              }`}
            ></span>
            <span className="text-xs text-gray-600 font-medium">
              {dorm.available_rooms > 0 ? 'ว่าง' : 'เต็ม'}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            ...
            <span className="text-xs text-gray-700">
              ...
              {Number(dorm.review_count).toLocaleString()})
            </span>
          </div>
          <p className="font-bold text-base">
            {Number(dorm.price_min).toLocaleString()}/เดือน
