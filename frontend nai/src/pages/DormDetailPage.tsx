...
import { getDormById, addToFavorites, removeFromFavorites, checkIsFavorite } from '../services/dormService';
import type { DormDetail, DormRoom } from '../services/dormService';
...
export default function DormDetailPage() {
    ...
    useEffect(() => {
        const fetchDormDetails = async () => {
            ...
            try {
                ...
                // เรียก API ดึงข้อมูลหอพัก
                const data = await getDormById(id);
                console.log("Received dorm data:", data);
                console.log("Room types:", data.rooms ? data.rooms.map(r => r.roomType) : 'No rooms data');
                setDorm(data);
                
                // เช็คว่าหอพักนี้อยู่ในรายการโปรดหรือไม่
                if (isAuthenticated) {
                ...
                setError(error.response?.data?.message || 'ไม่สามารถโหลดข้อมูลหอพักได้');
                setLoading(false);
            }
        };
    ...
    const getUniqueRoomTypes = () => {
        if (!dorm?.room_types) return [];
        const types = new Set(dorm.room_types.map(room => room.room_type_name));
        return Array.from(types);
    };

    const getFilteredRooms = () => {
        if (!dorm) return [];
        if (selectedRoomTypeFilter === 'all') return dorm.room_types;
        return dorm.room_types.filter(room => room.room_type_name === selectedRoomTypeFilter);
    };
    ...
    // ✅ เชื่อม API: ปุ่มกดถูกใจ
    const handleFavoriteToggle = async () => {
        if (!isAuthenticated) {
            if (window.confirm('กรุณาเข้าสู่ระบบก่อนเพิ่มหอพักโปรด\nคุณต้องการไปหน้าเข้าสู่ระบบหรือไม่?')) {
                navigate('/login');
            }
            return;
        }
        ...
        try {
            ...
            if (isFavorite) {
                ...
                setIsFavorite(false);
                alert('ลบออกจากหอพักโปรดแล้ว');
            } else {
                await addToFavorites(id);
                setIsFavorite(true);
                alert('✓ เพิ่มหอพักนี้เป็นที่โปรดแล้ว');
            }
        } catch (error: any) {
    ...
    const handleRoomDetailClick = (roomTypeName: string) => {
        // หาข้อมูลรูปแบบห้องพักที่ตรงกัน
        const roomType = dorm?.room_types?.find(rt => rt.room_type_name === roomTypeName);
        if (!roomType) return;
        
        // ส่ง ID ของรูปแบบห้องพักไปยังหน้ารายละเอียด
        navigate(`/dorm/${id}/room-type/${roomType.room_type_id}`);
    };
        ...
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-center">
                    ...
                    <div className="text-xl text-gray-600 mb-4">{error || 'ไม่พบข้อมูลหอพัก'}</div>
                    <button 
            ...
            <div>
                <div className="flex flex-col gap-4 mb-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        >
                            ทั้งหมด ({dorm.room_types.length})
                        </button>
                        {uniqueRoomTypes.map((roomType: string) => {
                            const count = dorm.room_types.filter(r => r.room_type_name === roomType).length;
                            return (
                                <button
                                    key={roomType}
                ...
                <div className="grid grid-cols-2 gap-6">
                    {filteredRooms.map((room) => (
                        <div key={room.room_type_id} className="bg-gray-100 rounded-lg overflow-hidden hover:shadow-lg transition">
                            <div className="p-4">
                                <div className="text-xl font-bold mb-2">{room.room_type_name}</div>
                                <div className="text-sm text-gray-600 mb-2">{room.room_type_desc}</div>
                                <div className="text-sm text-gray-700 mb-2">
                                    จำนวนผู้พักสูงสุด: {room.max_occupancy} คน
                                </div>
                                <div className="text-sm text-gray-700 mb-2">
                                    เงินประกัน: ฿{room.deposit_amount.toLocaleString()}
                                </div>
                                <div className="text-lg font-bold text-purple-600 mb-2">
                                    ฿{room.rent_per_month.toLocaleString()}/เดือน
                                </div>
                                <div className="text-sm text-gray-600 mb-4">
                                    ฿{room.rent_per_day.toLocaleString()}/วัน
                                </div>
                                <div className="flex justify-between items-center mb-4">
                                    <div className="text-sm">
                                        ห้องทั้งหมด: {room.total_rooms}
                                    </div>
                                    <div className="text-sm text-green-600">
                                        ห้องว่าง: {room.available_rooms}
                                    </div>
                                </div>
                                <button
                                    onClick={() => handleRoomDetailClick(room.room_type_name)}
                                ...
                                >
                                    {room.available_rooms === 0 ? 'ไม่มีห้องว่าง' : 'ดูรายละเอียด'}
                                </button>
                            </div>
                        </div>
                    ))}
            ...
            <div className="w-64 bg-white border-r border-gray-200 fixed h-full left-0 top-0 overflow-y-auto">
                <div className="p-6">
                    ...
                    <div className="mt-6 flex items-center gap-3 px-4 py-3 bg-gray-100 rounded-lg">
                        <img src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&h=100&fit=crop" alt="Profile" className="w-10 h-10 rounded-full" />
            ...
            <div className="w-96 fixed right-0 top-0 h-full overflow-y-auto bg-gray-50 border-l border-gray-200 p-6">
                <div className="space-y-6">
                    ...
                    >
                        ...
                        {isFavorite ? 'ลบออกจากหอพักโปรด' : 'เพิ่มเป็นหอพักโปรด'}
                    </button>

                    <button 
                        onClick={() => {
                            if (!isAuthenticated) {
                                if (window.confirm('กรุณาเข้าสู่ระบบก่อนจองหอพัก\nคุณต้องการไปหน้าเข้าสู่ระบบหรือไม่?')) {
                                    navigate('/login');
                                }
                                return;
                            }
                        ...
                        className="w-full px-4 py-4 bg-purple-600 text-white rounded-lg font-medium text-lg hover:bg-purple-700 transition cursor-pointer"
                    >
                        จองหอพัก
                    </button>
                </div>
