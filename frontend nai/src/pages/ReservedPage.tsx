        ...
        if (booking.status === 'รอการชำระเงินมัดจำ') {
            buttons.push(
                <button
                    ...
                    onClick={async () => {
                        try {
                            const response = await fetch(`http://localhost:3001/api/bookings/${booking.id}/pay-deposit`, {
                                method: 'POST',
                                credentials: 'include',
                                headers: {
                            ...
                            const updatedResponse = await fetch('http://localhost:3001/api/bookings/my-bookings', {
                                credentials: 'include'
                            });
        ...
        if (booking.status === 'รอการยืนยันจากเจ้าของหอพัก' || booking.status === 'รอการชำระเงินมัดจำ') {
            buttons.push(
                <button
                    ...
                    onClick={async () => {
                        try {
                            const response = await fetch(`http://localhost:3001/api/bookings/${booking.id}/cancel`, {
                                method: 'POST',
                                credentials: 'include',
                                headers: {
                                    'Content-Type': 'application/json'
